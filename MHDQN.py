...
def getReward(prev_pos, current_pos, orientation):
    ...
    """
    ##  Reward for jumping...
    position_reward = 1 if (current_pos[2] - 0.3) >= 0 else -1
...
def updateRewardFig(figure, ax, line, x_reward, y_reward, max_len=50):
    if len(x_reward) > max_len:
        x_reward = x_reward[-max_len:]
        y_reward = y_reward[-max_len:]
    ax.set_xlim((min(x_reward) - 2, max(x_reward) + 2))
    ax.set_ylim((min(y_reward) - 2, max(y_reward) + 2))
    line.set_xdata(x_reward)
    line.set_ydata(y_reward)
    figure.canvas.draw()
...
x_reward = [0]
y_reward = [0]
(line1,) = ax.plot(x_reward, y_reward, "b-o")
ax.set_xlim((0, 50))
ax.set_ylim((-2, 2))
...
for i in range(5000000):
    ...
    if not (i % 10):
        ...
        ##  To update reward plot...
        x_reward.append(i)
        y_reward.append(reward)
        updateRewardFig(figure, ax, line1, x_reward, y_reward)
    else:
        p.stepSimulation()
